...
class DynamicForm extends React.Component {
  ...
  getResponse() {
    ...
    axios.get(this.urlApiData).then(data => {
        ...
        <div>
          <Grid container spacing={2}>
            <Grid item xs={6}>
              <TextField
                id="standard-basic"
                label="TIVO manufacturingInfo"
                value={
                  this.state.data.response.data.TIVO.deviceInfoSearchResponse
                    .manufacturingInfo.type
                }
              />
            </Grid>
            <Grid item xs={6}>
              <TextField
                id="standard-basic"
                label="LDAP order event"
                value={
                  this.state.data.response.data.LDAP.orderCompletedEvent
                    .srqAction
                }
              />
            </Grid>
            ...
            <Grid item xs={6}>
              <TextareaAutosize
                ...
                placeholder={JSON.stringify(
                  this.state.data.response.data.CUBIWARE
                )}
              />
            </Grid>
            <Grid item xs={6}>
              <TextareaAutosize
                ...
                rowsMax={9}
                placeholder={JSON.stringify(
                  this.state.data.response.data.LINPUB
                )}
              />
            </Grid>
          </Grid>
